...
func (cg *ConsumerGroup) makeMemberProtocolMetadata(in []joinGroupResponseMemberV1) ([]GroupMember, error) {
	...
	for _, item := range in {
		reader := newBytesReader(item.MemberMetadata)

		metadata := groupMetadata{}
		metadata.readFrom(reader)

		if reader.err != nil || reader.n != 0 {
			return nil, fmt.Errorf("unable to read metadata for member, %v: %v", item.MemberID, reader.err)
		}
...
func (cg *ConsumerGroup) syncGroup(conn coordinator, memberID string, generationID int32, memberAssignments GroupMemberAssignments) (map[string][]int32, error) {
	...
	reader := newBytesReader(response.MemberAssignments)
	assignments := groupAssignment{}
	assignments.readFrom(reader)
