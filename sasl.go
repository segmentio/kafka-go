...
// calling Start at which point the initial bytes should be sent to the server.
// The caller then loops by passing the server's response into Next and then
// sending Next's returned bytes to the server.  Eventually either Next will
// indicate that the authentication has been successfully completed or an error
// will cause the state machine to exit prematurely.
//
