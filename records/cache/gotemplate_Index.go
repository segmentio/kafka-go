...
// Code generated by gotemplate, then edited to add a few optimizations:
// - change the position of the empty value field so it's not the last one
// - remove indirection of comparator function, use `less` method instead
...
func newIndex() *index {
	...
	return &index{beginNode: endNode, endNode: endNode}
}
...
func (t *index) findNode(id *entry) *nodeIndex {
	...
	for current != nil {
		switch {
		case id.less(current.key):
			current = current.left
		case current.key.less(id):
			current = current.right
		default:
			return current
		}
